...
//         <h3 className="text-2xl font-bold text-[#41A4FF]">Travel World</h3>
//       </div>
//       {/* <!-- left header section --> */}
...
//   // Check if user is in localStorage on initial render
//   useEffect(() => {
//     const storedUser = localStorage.getItem("user");
//     if (storedUser) {
//       // User exists in localStorage, update context state accordingly
//       // Optionally set the user in context here if you use localStorage to store user data
//     }
//   }, []);
...
//                   <Menu.Item key={index}>
//                     {({ active }) => (
//                       <Link className={classNames(active ? "bg-gray-100 text-gray-900" : "text-gray-700", "block px-4 py-2 text-sm")} to={item.path}>
//                         {item.display}
//                       </Link>
//                     )}
//                   </Menu.Item>
...
//               <div>
//                 <Menu.Button className="flex rounded-full bg-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800">
//                   <img className="h-8 w-8 rounded-full" src={user?.img || JSON.parse(localStorage.getItem("user"))?.img} alt="" />
//                 </Menu.Button>
//               </div>
...
//                 <Menu.Items className="absolute right-0 z-10 mt-2 w-40 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
//                   <div className="py-1">
//                     <h2 className="block px-4 py-2 text-sm text-[#41A4FF]">{user?.name || JSON.parse(localStorage.getItem("user"))?.name}</h2>
//                     <Menu.Item>
//                       {({ active }) => (
//                         <Link className={classNames(active ? "bg-gray-100 text-gray-900" : "text-gray-700", "block px-4 py-2 text-sm")} to="/profile">
//                           Profile
//                         </Link>
//                       )}
//                     </Menu.Item>
//                     <Menu.Item>
//                       {({ active }) => (
//                         <button type="button" onClick={handleLogout} className={classNames(active ? "bg-gray-100 text-gray-900" : "text-gray-700", "block w-full px-4 py-2 text-left text-sm")}>
//                           Sign out
//                         </button>
//                       )}
//                     </Menu.Item>
...
//         ) : (
//           <>
//             <Link to="/login" className="px-4 py-2 text-white font-bold bg-[#41A4FF] text-center hover:bg-blue-500 cursor-pointer rounded-md">
//               Sign in
//             </Link>
//             <Link to="/register" className="px-4 py-2 text-white font-bold bg-gray-800 text-center hover:bg-gray-600 cursor-pointer rounded-md">
//               Sign up
//             </Link>
//           </>
//         )}
...
//       {/* Mobile Menu */}
//       <div onClick={handleNav} className="block md:hidden">
//         {nav ? <AiOutlineMenu size={20} style={{ color: "black" }} /> : <AiOutlineClose size={20} style={{ color: "black" }} />}
//       </div>
//       <div className={!nav ? "fixed left-0 top-0 w-[60%] h-full border-r border-r-gray bg-white ease-in-out duration-500 md:hidden" : "fixed left-[-100%]"}>
//         <h1 className="text-2xl font-medium text-blue-500 m-8">Travely</h1>
//         <ul className="p-4 mt-20">
//           <li className="p-4 border-b border-gray-600">
//             <Link to="/">Home</Link>
...
//                   <img className="h-8 w-8 rounded-full" src={user?.img || JSON.parse(localStorage.getItem("user"))?.img} alt="" />
//                 </Menu.Button>
//                 <h2 className="block py-2 text-sm text-[#41A4FF]">{user?.name || JSON.parse(localStorage.getItem("user"))?.name}</h2>
//               </div>
//               <Transition as={Fragment} enter="transition ease-out duration-100" enterFrom="transform opacity-0 scale-95" enterTo="transform opacity-100 scale-100" leave="transition ease-in duration-75" leaveFrom="transform opacity-100 scale-100" leaveTo="transform opacity-0 scale-95">
//                 <Menu.Items className="absolute left-0-0 z-10 mt-2 w-40 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
//                   <div className="py-1">
//                     <Menu.Item>
//                       {({ active }) => (
//                         <Link className={classNames(active ? "bg-gray-100 text-gray-900" : "text-gray-700", "block px-4 py-2 text-sm")} to="/">
//                           Profile
//                         </Link>
//                       )}
//                     </Menu.Item>
...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    const storedUser = localStorage.getItem("travely_user");
    if (storedUser) {
          ...
          <>
            ...
            <Menu as="div" className="relative inline-block text-left">
              <div>
                <Menu.Button className="flex rounded-full bg-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800">
                  {/* {user.name} */}
                  <img className="h-8 w-8 rounded-full" src={userFromLocalStorage?.img || user?.img} alt="" />
                </Menu.Button>
              </div>
              ...
              >
                <Menu.Items className="absolute right-0 z-10 mt-2 w-40 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                  <div className="py-1">
                    <h2 className="block px-4 py-2 text-sm text-[#41A4FF]">
                      {userFromLocalStorage?.name || user?.name}
                    </h2>
                    <Menu.Item>
                      {({ active }) => (
                        <Link
