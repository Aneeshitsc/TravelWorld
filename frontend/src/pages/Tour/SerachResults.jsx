...
const SerachResults = () => {
  ...
  useEffect(() => {
    const getTours = async () => {
      try {
        const response = await axios.get(`http://localhost:4000/api/tours/getTour/${destination}`);
  ...
  return (
    <div>
      <div style={image} className="relative overflow-hidden bg-no-repeat bg-cover">
        <div className="flex h-full items-center justify-center text-center">
          <div>
            <h2 className="mb-5 text-6xl font-bold text-black" style={{ fontFamily: "Poppins, sans-serif", fontWeight: "bolder" }}>
              Search Results
            </h2>
      ...
      <nav className="bg-grey-light w-full rounded-md pl-20 pt-10">
        <ol className="list-reset flex">
          ...
          <li><AiOutlineRight className="mt-1 mx-2" /></li>
          <li>
            <Link to="#" className="text-primary hover:text-primary-600">Tour Packages</Link>
          </li>
          <li><AiOutlineRight className="mt-1 mx-2" /></li>
          <li className="text-neutral-500">Explore World</li>
        </ol>
      </nav>
      ...
      <div className="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8">
        ...
        {loading ? (
          <div className="text-center text-lg">
            <div
              className="inline-block h-8 w-8 animate-[spinner-grow_0.75s_linear_infinite] rounded-full bg-current align-[-0.125em] opacity-0"
              role="status"
            >
                ...
                {filteredTours.map((tours) => (
                  ...
                  >
                    ...
                    <div className="mt-4 flex justify-between p-3">
                      <h3 className="text-2xl font-bold text-gray-700">
                        <Link to={`/tours/${tours._id}`}>
                          <span className="absolute inset-0 rounded-t-3xl" />
                          {tours.name}
                        </Link>
                        <p className="text-lg font-medium text-gray-900">
                          {tours.duration} days
                    ...
                    <div className="flex flex-row mr-2 space-x-3 justify-between">
                      ...
                      <Link to={`/tours/${tours._id}`}>
                        <button className="mb-2 inline-block rounded bg-primary px-4 pb-2 pt-2.5 text-xs font-medium uppercase text-white hover:bg-primary-600 transition">
                          View Details
                        </button>
                      </Link>
                    </div>
